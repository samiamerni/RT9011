...
def compare(drink,sugar):
    ...
    if drink == sugar :
        ...
        print('il est pas possible de sélectionner une boisson et un nombre de sucre indiqué')   
        print('test echoué')
        return False
...
if __name__ == "__main__":
    ...
    msgsend = sendto(UDPServerSocket,bytes.fromhex("2205"))
    validatesuggar =recerivefrom(UDPServerSocket,msgsend,idreq_2)
    sugarSelected = checkIfCanSelectSuggar(bytesAddressPair,msgsend,idreq_2)
    logfile() 
    compare(drinkSelected,sugarSelected)