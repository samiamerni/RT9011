...
def checkIfCanSelectDrink(bytesAddressPair,msgsend,idreq):
    if bytesAddressPair != "inconc":
        ...
        if coderesult == 1 :
            ...
            print('On ne peut pas sélectionné une autre boisson car il y\'a une boisson qui doit etre récupéré)')   
            datasuccess="success"
            dataverdic="pass"
            resultsucess= [datasuccess,datasuccess,dataverdic,msgreturn_,v1_]
            listedata_1.extend(resultsucess)
...
if __name__ == "__main__":
    ...
    logfile()
    listedata_1 = []
    ## récupérer la boisson false
    ## selectionner une boisson
    msgsend = sendto(UDPServerSocket,bytes.fromhex("2101"))
    bytesAddressPair =recerivefrom(UDPServerSocket,msgsend,idreq_4)
    selectdrinknotpossible = checkIfCanSelectDrink(bytesAddressPair,msgsend,idreq_4)
    logfile()
