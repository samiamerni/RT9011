...
def checkGetDrink(bytesAddressPair,msgsend,idreq):
    if bytesAddressPair != "inconc":
        ...
        if getdrink == 1 :
            ...
            print("la boisson ne peut etre recupérer récupérer")  
            dataverdic="error"
            resulterror = [dataverdic,dataverdic,dataverdic,msgreturn_,v1_]
...
def decodeprice(bytesAddressPair,stringjson,msgsend,idreq):
    if bytesAddressPair != "inconc":
        ...
        if stringjson["changeText"] == stringresult or stringjson["chooseDrinkText"] == stringresult or stringjson["drinkMadeText"] == stringresult or stringjson["noBucket"] == stringresult :
            print('le texte affiché est :', stringresult)
            datasucess="success"
            dataverdic="pass"
            resultsuccess= [stringresult,datasucess,dataverdic,msgreturn_,v1_]
            listedata_1.extend(resultsuccess)
            return stringresult
        else:
            print('le prix de la boisson est :',stringresult)
            dataerror="error"
            dataverdic="error"
            resulterror= [stringresult,dataerror,dataverdic,msgreturn_,v1_]
            listedata_1.extend(resulterror)
            return stringresult
    else:
        print('il est impossible de Conclure')
...
if __name__ == "__main__":
    ...
    validatedrink =recerivefrom(UDPServerSocket,msgsend,idreq_5)
    stringjson = openconfigfile()
    textafficher= decodeprice(validatedrink,stringjson,msgsend,idreq_5)
    logfile()
