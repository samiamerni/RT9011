...
def checkIfCanSelectSuggar(bytesAddressPair,msgsend,idreq):
   if bytesAddressPair != "inconc":
        ...
        if coderesult == 1 :
            ...
            print('le sucre n\'a pas été selectionnée car la quantité dépasse 10')    
            datasuccess="success"
            dataverdic="pass"
            resultsucess = [datasuccess,datasuccess,dataverdic,msgreturn_,v1_]
            listedata_2.extend(resultsucess)
...
def compare(drink,sugar):
    ...
    if drink == sugar :
        print('Selection boisson possiblité de selectionner un nombre de sucre inférieur à 10')
        print('test reussi')
        return True
    else:
        print('il n\'est pas possible de sélectionner une boisson avec le nombre de sucre indiqué')   
        print('test echoué','car le nombre de sucre doit etre inférieur à 10')
        return False
...
if __name__ == "__main__":
    ...
    msgsend = sendto(UDPServerSocket,bytes.fromhex("2211"))
