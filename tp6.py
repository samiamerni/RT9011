...
def checkIfCanSelectSuggar(bytesAddressPair,drinkselected):
    ...
    if drinkselected == 1 and codesuggar == 0:
        print("la boisson soap a été selectionné et le sucre ne peut pas etre selectionner")
        return True
    elif  drinkselected == 1 and codesuggar == 1: 
        print("la boisson soap a été selectionné et le sucre a ete ajouter ce qui n'est pas normal")
        return False
    else:
        print("il se peur que la boisson n'a pas ete valisdé")
        return False
...
if __name__ == "__main__":
    ...
    sendto(UDPServerSocket,bytes.fromhex("2207"))
